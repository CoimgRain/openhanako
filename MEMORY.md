...
- 目标：修复合并上游后子 Agent 会话点开正文为空、控制台 `/api/sessions/messages` 500 的问题。
- 背景：用户截图显示左侧子任务列表与运行状态可见，但点击子任务后聊天区为空；直接复现截图中的 subagent session 请求返回 `{"error":"allMessages is not defined"}`。
